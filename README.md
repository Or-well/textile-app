...
Textile 是一个面向汉化组和翻译协作团队的本地优先项目管理工具。项目正文、成员、任务、术语、评论和协作记录保存在用户选择的本地项目文件夹中，也可以打包为 `.hproj` 项目文件。
...
- 管理上下文、术语、评论、任务、成员、角色和个人权限。
- 查看翻译、校对、审核和综合进度。
- 导出普通修改包、任务修改包、维护修改包和签名项目更新包。
- 预览、验签、检查完整性、处理冲突并导入修改包。
- 导出 JSON、TXT、CSV、KS 成品和检查报告。
- 导出或打开包含项目数据的 `.hproj` 项目包。
- 支持 Web/PWA 更新提示，并预留 Tauri Updater 桌面自动更新。
...
1. 在项目启动中心创建项目，或打开已有 Textile 项目文件夹。
2. 使用项目成员名和密码登录。
3. 在“文件”页添加源文件，或更新源文件、导入已有译文。
4. 进入词条编辑页完成翻译、校对、审核、上下文和评论工作。
5. 在“任务”“术语”“统计”页管理协作过程。
6. 普通成员导出自己的修改包并交给负责人。
...
`.hproj` 是包含上述项目数据的 ZIP 类项目包，可用于备份、迁移或在另一台设备打开。当前版本打开 `.hproj` 后，修改先保存在本次运行的内存项目中；要永久保留修改，必须再次执行“导出 Textile 项目备份”，生成新的 `.hproj` 文件。

## 协作方式

项目协作以修改包为主，不使用项目 Git 同步：

- 普通修改包：成员导出自己的词条、评论、术语、任务等修改。
- 任务修改包：只导出所选任务范围内的修改。
