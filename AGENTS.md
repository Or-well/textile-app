...
Vue 页面和组件不得直接读写项目文件。新增功能前先查找现有 service；需要文件访问时由 service 调用 `projectFs.ts`。不要为了一个功能一次性重写核心存储、修改包、权限或状态模块。
...
- 监听 `popstate`。
- 持有当前项目、当前用户和项目摘要状态。
- 打开项目后调用各 service 的 `set*ProjectRoot()`。
...
- `exports/`、`changes/` 会随项目备份打包，但当前多数导出通过浏览器下载，不自动写入这些目录。
...
- 项目根目录通过现有 `set*ProjectRoot()` 或明确参数传入；不要新增第二套全局根目录机制。
