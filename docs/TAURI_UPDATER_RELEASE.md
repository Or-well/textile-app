...
6. 创建或打开一个测试项目。
7. 测试基本打开、保存和关闭。
8. 从工作台点击“使用手册”，确认系统默认 PDF 阅读器可以打开随包的 `manual.pdf`。
9. 至少导出一个生成文件，确认出现系统保存窗口、文件写入所选路径，并且取消保存时应用不会显示导出成功或推进相关状态。
10. 使用测试项目验证一次发布密钥轮换：新私钥先保存、过渡包保存后项目才换钥，且 `changes/transitions/` 中存在可重新保存的归档。
11. 使用没有加载旧私钥的负责人测试“私钥丢失，重建信任”，确认生成新 `.hproj`、`trust_epoch` 增加，并且旧副本拒绝跨代项目更新。
...
5. 确认界面显示新版本号。
6. 点击下载或安装更新。
7. 在没有修改任何表单时停留在文件、设置或导入导出页面，确认不会仅因页面位置而提示等待。
8. 修改一处译文或项目设置但不要保存，确认应用显示具体的未保存草稿原因并暂缓安装。
9. 保存或放弃草稿，确认等待状态解除。
10. 执行修改包导出或导入，确认操作期间显示具体原因，成功或失败结束后等待状态都会解除。
11. 点击“安装并重启”。
12. 重启后检查版本号已经更新。
13. 打开原有测试项目，确认项目数据仍存在。
...
- 有实际未保存草稿，或正在执行写入、导入或导出。
- 应显示具体等待原因，桌面版不使用“刷新”描述安装过程。
- 保存、放弃草稿或操作结束后才允许安装。
...
- [ ] 更新后版本号正确。
- [ ] 用户项目数据未受影响。
- [ ] 未保存草稿或实际导入导出期间的更新阻止正常，普通页面停留不会误报。
...
`tauri-apps/tauri-action@v0.6.2` 是当前确认存在的 action tag。以后升级前应先到 `tauri-apps/tauri-action` 的 tags 页面确认目标 tag 存在，不要只根据 README 示例改成不存在的版本。
