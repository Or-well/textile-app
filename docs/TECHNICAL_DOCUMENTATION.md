...
- 普通成员修改包仍只收集 `created_by` 为当前成员且仍存在的术语，不能表达术语删除；协作项目中的术语批量删除需要由负责人通过完整项目更新流程分发。这是既有修改包模型的限制，不由批量页面另行定义协议。

### `termBatch.ts`

`termBatch.ts` 管理术语的统一词性、大小写规则和批量删除。预检与执行都会重新读取 `terms/terms.jsonl`，按 `term.update` 或 `term.delete` 权限校验，跳过不存在或无变化的术语。实际变更一次性写回完整术语文件，并沿用 `terms.ts` 的排序、规范化和缓存更新。
