...
src/
  ...
  pages/
    ...
    TermsPage.vue
  components/
    BatchPreviewDialog.vue
    BatchSelectionBar.vue
    ChangePreview.vue
    CommentEditor.vue
    CommentListItem.vue
    CommentPanel.vue
...
- `collaboration.require_signed_change_packages`：要求普通、任务、维护修改包和项目更新包带有有效成员签名。新项目默认 `true`；旧项目缺失该字段时按 `false` 兼容读取。密钥轮换、撤销和公钥恢复等密钥信任过渡不受该兼容开关影响，始终必须签名；负责人转让在非强制签名项目中可使用未签名的人工信任项目更新包。
...
logs/events.jsonl               # 当前活动片，最多 1000 条或 256 KiB
```

旧项目只有 `logs/events.jsonl` 时无需迁移，仍可直接读取。活动片追加后超过 1000 条或 256 KiB 时，达到任一上限的前段事件会在同一个 `ProjectWritePlan` 中写入下一个归档片，剩余事件继续保存在活动片；单个事件本身超过 256 KiB 时允许独占一片。普通追加只列出归档文件并读取活动片，不读取或重写已经封存的 chunk。同一保存流程需要先查询近期事件再追加事件时，应复用 `ProjectEventLogTail`，避免重复读取活动片。
...
`fileBatch.ts` 管理既有文件的批量隐藏、取消隐藏、锁定、解锁、分组调整和删除。预检与执行都会重新读取磁盘 `project.json`、确认项目 ID 和对应权限，并把不存在或已经是目标值的文件列为跳过项。

批量元数据更新只写一次 `project.json`，并为每个实际变化的文件追加带同一 `batch_id` 的文件事件。批量删除会先计算删除后的剩余引用：仍被其他文件引用的 source 或 entries 路径不得删除；同一路径最多加入写入计划一次。源文件、词条文件、目录、项目索引和全部文件事件通过一个 `ProjectWritePlan` 提交，成功后才清理受影响文件的词条缓存。

文件分组继续使用 `ProjectFile.folder?: string`，不新增独立分组表或磁盘目录。`utils/fileGroups.ts` 负责分组名称去除首尾空白、现有分组汇总和筛选判断；空名称统一视为未分组。`FilesPage` 只维护分组筛选、折叠和对话框状态，单文件调整、整组重命名、整组移出以及多选调整都复用 `fileBatch.ts`。重命名为已有名称等价于把原组全部文件移动到目标组，因此不会产生重复分组；没有文件引用的分组不会保存。

创建文件时如果同时指定分组，`project.ts` 会在 `FILE_CREATE` 之外再次要求 `FILE_MANAGE_FOLDER`。通用文件 patch 只要包含 `folder`，也按 `FILE_MANAGE_FOLDER` 校验，防止页面或未来调用方绕过分组权限。
...
`termBatch.ts` 管理术语的统一词性、大小写规则和批量删除。预检与执行都会重新读取 `terms/terms.jsonl`，按 `term.update` 或 `term.delete` 权限校验，跳过不存在或无变化的术语。实际变更一次性写回完整术语文件，并沿用 `terms.ts` 的排序、规范化和缓存更新；批量删除会同步写入 `changes/term-deletions.jsonl`，用于普通修改包表达删除。
...
- 普通修改包将 `due_at` 和 `due_time_zone` 视为任务受保护字段。维护修改包实际修改截止时间时必须同时提供绝对时间和有效 IANA 时区；项目更新包可继续承载旧任务，由接收方后续明确迁移。
...
- `config/permissions.json`：角色顺序、角色标签、权限分组展示和默认角色权限。
- `model/permissions.ts`：权限 action、owner 锁定权限、配置 schema 校验和类型化导出。
...
启动时 `model/permissions.ts` 会校验 `config/permissions.json`：角色必须完整且无未知项，权限 action 必须存在于 `PERMISSION_ACTIONS`，默认角色权限必须出现在权限目录中，owner 默认权限必须包含系统锁定权限。配置错误会直接抛出“权限配置错误”，避免静默丢失权限。

项目可以通过 `settings.role_permissions` 覆盖默认角色权限。旧项目如果完全没有新的修改包 action，兼容逻辑会补足新协议所需权限。缺少 `permission_schema_version` 的旧权限配置会补充词条锁定和隐藏默认权限；保存后升级为当前 schema 并严格服从用户配置。
...
| translator | 翻译、争议/批注/上下文处理、普通修改包 |
| proofreader | 校对、回退、争议/批注/上下文处理、普通修改包 |
| reviewer | 审核、争议/批注/上下文处理 |
...
- 当前成员创建或最后编辑的术语，以及当前成员产生的术语删除记录。
- 当前成员创建或负责的任务。
- 当前成员事件。
...
默认普通协作角色可以导入维护修改，便于接收负责人或维护人员分发的项目维护包；导出维护修改和危险导入仍只授予维护或管理角色。
...
- 当 `settings.collaboration.require_signed_change_packages` 为 `true` 时，member_changes、task_changes、maintenance_changes 和 project_update 必须由当前成员签名；缺少签名权限、公钥已撤销、未生成公钥或私钥未加载都会在 `exportChangePackage()` 中拒绝。
- 当该设置为 `false` 或旧项目缺失该设置时，UI 允许成员选择是否签名；选择签名时 `exportChangePackage({ sign: true })` 仍会要求签名权限、有效公钥和已加载私钥，不能静默降级为未签名包。普通 project_update 也遵守该选项。
...
- 强制签名项目中必须签名；非强制签名项目中可以未签名发布。密钥轮换、撤销和公钥恢复等密钥信任过渡 project_update 始终必须签名；负责人转让在非强制签名项目中可以未签名发布。
- manifest 发布人必须是当前项目中有发布项目更新权限的成员；已签名包的签名身份还必须通过当前可信公钥验证。
...
- content_hash 重新计算。
- signature 状态计算。
- 当前项目要求签名时，member_changes、task_changes、maintenance_changes 和 project_update 必须是 valid 签名；危险导入不能绕过签名要求。
- package 类型和风险。
...
- 普通修改包中的术语新增、编辑和删除按导出者当前术语权限校验。`terms/terms.jsonl` 只能提交导出者创建或最后编辑的术语；`term-changes/deletions.jsonl` 的删除人必须等于 `manifest.user_id`。
...
9. 普通包词条只合并允许的译文、工作流、上下文和争议字段，`updated_by` 使用包 manifest 用户；普通包批注只合并允许的状态字段或按 `comment.deleted` 事件删除；普通包术语按 `terms/terms.jsonl` 合并新增/编辑并按 `term-changes/deletions.jsonl` 删除；普通包任务只合并允许的执行状态。任务准备进入 `submitted` 时，如果任务范围内仍有词条选择保留或跳过，整次导入停止，避免任务状态和实际采用内容脱节。
...
12. 去重合并实际采用内容对应的其余包内 events；被保留或跳过内容的事件不写入主项目。导入日志记录包内、应用、已一致和保留/跳过词条数量。
...
冲突初始状态为未选择，页面提供“全部使用修改包”和“全部保留当前项目”批量操作；仍有未选择项时不能应用。
...
术语新增/编辑冲突支持保留当前项目、使用修改包版本或手动处理；术语删除冲突只支持保留当前项目或使用修改包删除。
...
负责人转让先由 `prepareOwnerTransfer()` 生成转让后的公开成员快照。强制签名项目中，成员管理 UI 验证新负责人的交接证明，再用当前旧负责人签名人快照导出 signed `project_update`；确认过渡包已保存后通过统一项目更新过渡 service 提交，导入端仍使用当前项目里旧负责人的公钥验证该过渡包。非强制签名项目中，UI 不要求当前负责人私钥、新负责人公钥或交接证明，导出 unsigned `project_update`，保存确认后通过同一提交边界写入成员、事件、归档和 revision。

成员管理 UI 通过统一的 `ownerTransferBlockReason` 计算不可转让原因。强制签名项目覆盖权限、项目根目录、目标成员、目标公钥、交接证明和当前负责人可用私钥；非强制签名项目只阻止权限、项目根目录、项目配置和目标成员问题。按钮禁用时必须同步展示该原因，不能只留下无响应按钮。

### 签名信任过渡

`signingTrustTransition.ts` 是发布密钥轮换、公钥恢复、发布密钥撤销和负责人转让项目更新包的统一提交边界。
...
强制签名负责人转让使用 `textile.owner_transfer_key_proof` JSON 文件作为交接证明，签名内容绑定 project ID、当前 revision、成员 ID、公钥和 key ID；验证时还要求生成时间不超过 24 小时。非强制签名项目不要求该证明。
...
| `FilesPage` | project、fileBatch、entries、permissions、stats |
...
| `TermsPage` | terms、termBatch、permissions |
...
Tauri Updater 只负责更新 Textile 程序本身，不负责更新某个翻译项目。项目数据仍走 Textile 内部的 `.hproj`、普通修改包和项目更新包流程。
...
- 批注和争议跨文件写入不是事务。
- 日志只重写最多 1,000 条或 256 KiB 的活动片；任务、术语和单个数据 chunk 仍是全文件重写。
- 多标签页同时编辑同一项目没有锁和合并。
- 直接磁盘修改无法被权限系统阻止。
...
2. 在 `config/permissions.json` 的 `groups` 增加中文标签和展示分组。
3. 在 `config/permissions.json` 的 `default_role_permissions` 加入需要默认获得该权限的角色。
4. 如旧项目需要默认获得，扩展兼容逻辑。
...
- Tauri 桌面版的文件读取、写入、存在性检查、目录列表和删除直接调用既有的受限相对路径命令；浏览器文件夹与 `.hproj` 内存存储仍走统一 handle 接口。这里没有新增原生协议，也不绕过 Rust 侧根目录和相对路径校验。
- `ProjectWritePlan` 以有上限并发捕获回滚快照；已有文件只保存文件自身快照，新建文件才记录可能由写入隐式创建的父目录。应用顺序、写入校验和逆序回滚语义保持不变。
...
- 项目打开只等待结构、配置和成员读取；全项目统计在进入工作台后延迟约 500 ms，并以并发 2 后台加载，避免与首屏文件读取争用磁盘。
...
性能回归测试位于 `tests/unit/entryReadModel.test.ts`、`tests/unit/projectFs.test.ts`、`tests/unit/projectWritePlan.test.ts` 和 `tests/unit/eventLog.test.ts`，至少验证并发读取合并、热缓存复用、原生单请求访问、已有文件快照范围、日志尾复用和按字节分片。最低检查仍为：
